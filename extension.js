...
const ImageListURL =
  "https://commons.wikimedia.org/w/index.php?title=User:Jon_Harald_S%C3%B8by/wallpapers.json&action=raw&ctype=application/json";
const CommonsImageURLbase =
  "https://commons.wikimedia.org/w/api.php?format=json&action=query&prop=imageinfo&iiprop=url|extmetadata&iiextmetadatafilter=ImageDescription|Artist|LicenseUrl|LicenseShortName&titles=";
...
function log(msg) {
  if (
    ...
    commonsWallpaperIndicator._settings.get_boolean("debug-logging")
  )
...
const CommonsWallpaperIndicator = new Lang.Class({
  ...
  _init: function () {
    ...
    let gicon = Gio.icon_new_for_string(
      Me.dir.get_child("icons").get_path() + "/" + ICON + ".svg"
    );
    ...
    this.titleItem = new PopupMenu.PopupImageMenuItem(
      _("Awaiting refresh …"),
      "camera-photo-symbolic"
    );
    this.creatorItem = new PopupMenu.PopupImageMenuItem(
      _("Awaiting refresh …"),
      "face-monkey-symbolic"
    );
    this.licenseItem = new PopupMenu.PopupImageMenuItem(
      _("Awaiting refresh …"),
      "application-certificate-symbolic"
    );
    this.clipboardItem = new PopupMenu.PopupMenuItem(
      _("Copy image URL to clipboard")
    );
    this.refreshItem = new PopupMenu.PopupImageMenuItem(
      _("Refresh now"),
      "view-refresh-symbolic"
    );
    this.settingsItem = new PopupMenu.PopupImageMenuItem(
      _("Settings"),
    ...
    this.licenseItem.connect(
      "activate",
      Lang.bind(this, function () {
        if (this.licenselink) Util.spawn(["xdg-open", this.licenselink]);
      })
    );
    this.titleItem.connect(
      "activate",
      Lang.bind(this, function () {
        if (this.imageinfolink) Util.spawn(["xdg-open", this.imageinfolink]);
      })
    );
    this.clipboardItem.connect(
      "activate",
      Lang.bind(this, this._copyURLToClipboard)
    );
    ...
    this.actor.connect(
      ...
      Lang.bind(this, function () {
        ...
        this.clipboardItem.setSensitive(
          !this._updatePending && this.imageURL != ""
        );
        this.titleItem.setSensitive(
          !this._updatePending && this.imageinfolink != ""
        );
  ...
  _parseData: function (data) {
    ...
    if (imageinfo["thumburl"]) {
      ...
      let userPicturesDir = GLib.get_user_special_dir(
      ...
      log(
        "XDG pictures directory detected as " +
          userPicturesDir +
          " saving pictures to " +
          CommonsWallpaperDir
      );
      ...
      let file_info = file_exists
        ? file.query_info("*", Gio.FileQueryInfoFlags.NONE, null)
        : 0;

      if (!file_exists || file_info.get_size() == 0) {
        // file doesn't exist or is empty (probably due to a network error)
  ...
  _download_image: function (url, file) {
    ...
    request.connect(
      ...
      Lang.bind(this, function (message, chunk) {
        ...
        if (message.status_code == 200) {
          // only save the data we want, not content of 301 redirect page
...
function enable() {
  ...
  for (let monitorIdx in monitors) {
    ...
    log(
      "monitor " + monitorIdx + " -> " + monitor.width + " x " + monitor.height
    );
