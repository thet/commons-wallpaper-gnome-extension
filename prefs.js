...
function buildPrefsWidget() {
  ...
  log(
    "fileChooser filename/dirname set to '" +
      fileChooser.get_filename() +
      "' setting is '" +
      settings.get_string("download-folder") +
      "'"
  );
  ...
  resolutions.forEach(function (res) {
    // add res to dropdown list (aka a GtkComboText)
...
function validate_resolution() {
  ...
  if (resolution == "" || resolutions.indexOf(resolution) == -1)
    // if not a valid resolution
